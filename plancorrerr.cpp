 ...
 *
 * Caculating Clustering Measurement Errors for the HETDEX survey.
 *
 *
 ...
 *	Compile > g++ -Wall -I/usr/local/include -L/usr/local/lib -lgsl -lgslcblas -lm -O3 plancorrerr.cpp -o plancorrerr.bin
*/
...
// ruler input : decimal ra, decimal deg dec, decimal deg length
	public: 
		...
		void calculateShortTermError(void); // return permutation average of exp(- tau/tauzero)
...
void RulerScheduleTools::calculateShortTermError(void) { // setRulerScale before running this 
	int i,j;		
	double totalcorr,totalrandomcorr,tmpfactor,tmplength,tmprandomfactor;
...
//		cout << "RulerScheduleTools::calculateShortTermError -- Assigning Time Gaps.. done... " << endl;
		totalcorr=0.0;totalrandomcorr=0.0;
...
//			<< " ; random affected amount = " << totalrandomcorr << " ; fraction random = " << (totalrandomcorr/pr->total) << endl;
		...
		//write the results
		shorttermerrors[i] = (totalcorr/pr->total); shorttermrandomerrors[i] = (totalrandomcorr/pr->total); 
	}
